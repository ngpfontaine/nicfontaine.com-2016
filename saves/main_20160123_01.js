  ...
  var colorScrubInterval = setInterval(iterColor,200);;
    ...
    document.querySelector('#cmd-input-hidden').addEventListener('keydown', function (e) {
      ...
      if (e.which == 9) {
        ...
        var y = $(cmdInputCombo + '> .article-inner').scrollTop();
        $(cmdInputCombo + '> .article-inner').scrollTop(y + 10);
        ...
        if (y <= scrollContainerHeight) {
          ...
          // MOVE SCROLL-BAR TILL AT HEIGHT OF CONTAINER
          if (scrollHeightInc <= scrollContainerHeight) {
            scrollHeightInc += scrollContainerHeight/6;
          }

          $('.scroll-bar').css({'top': scrollHeightInc});
    ...
    $('#cmd-input-hidden').keyup(function(e) {
        ...
        if(e.keyCode === 13 &&  cmdInput !== "" && cmdInput) {
          ...
          // LAUNCH FULLSCREEN
            ...
            window.location = 'http://nicfontaine.com/dev/nicfontaine/?cmd=fullscreen';
          }
